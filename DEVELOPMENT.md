...
| 메모리 + 디스크 2단계 이미지 캐시 + in-flight 중복제거 | Kingfisher / SDWebImage |
...
- **유닛 테스트**: Swift Testing Framework, 179개 케이스, Domain + ViewModel + BookmarkCoordinator + CachedAsyncImageViewModel + DTO + SearchFlowController + SearchPrefetchCoordinator + SearchResultsStore 검증 중심 (`Unit/`)
...
스킴 검증은 `toImageItem()` 변환 시점에, Content-Type·크기 검증은 `ImageDownloader` actor 내부에서 수행합니다.
...
| `flatMapLatest` | `searchTask` 재생성 + `activeSearchID` 기반 늦은 응답 무시로 이전 요청 취소 |
...
- `ImageAnalyzer`: Vision API(`VNClassifyImageRequest`)로 이미지 키워드를 자동 추출하고, iOS 26+에서는 Apple Foundation Models로 사용자 언어의 자연어 설명문으로 변환해 `accessibilityLabel`에 반영. iOS 17~25에서는 영어 키워드 나열로 폴백. VoiceOver 활성화 시에만 실행하여 리소스 절약. 통합 테스트는 Vision ML 모델이 시뮬레이터에서 미지원되어 실기기 전용으로 구성.

#### OSLog 기반 로깅
- `Logger.network`, `Logger.imageLoader`, `Logger.imageAnalyzer`, `Logger.bookmark`, `Logger.presentation` 카테고리 분리 (각 카테고리는 사용하는 레이어에 정의)
