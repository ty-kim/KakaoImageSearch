...
- **메모리 압박 대응**: `ImageCache`의 `NSCache`는 `totalCostLimit`(50MB)으로 메모리를 제한하고 있지만, `UIApplication.didReceiveMemoryWarningNotification` 수신 시 디스크 캐시까지 능동적으로 정리하는 로직은 포함하지 않았다. 이미지가 핵심 콘텐츠인 앱에서 저사양 기기 대응을 위해 추가가 필요하다.

- **이미지 프리패치**: 페이지네이션 결과가 도착하면 다음 페이지 썸네일을 `TaskGroup`으로 병렬 선수 다운로드하면 스크롤 시 체감 버벅임을 줄일 수 있다. Swift Concurrency의 구조화된 동시성을 활용할 수 있는 자연스러운 확장 지점이지만, 과제 범위를 넘는다고 판단해 포함하지 않았다.

- **이미지 로드 우선순위 조절**: `LazyVGrid`에서 현재 뷰포트 안에 보이는 아이템은 `.high`, 뷰포트 밖 아이템은 `.low`로 URLSession task priority를 분리하면 사용자가 실제로 보는 이미지가 먼저 로드된다. 웹툰처럼 이미지가 핵심 콘텐츠인 서비스에서 UX에 직접 영향을 주는 개선이다.
