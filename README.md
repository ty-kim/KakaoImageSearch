...
| 검색 | 북마크 |
|------|--------|
| 키워드 입력 → 0.3초 debounce → 결과 표시 | 북마크 탭에서 저장 목록 확인 |
...
### 검색 Debounce
- `Task.sleep(for: .seconds(0.3))` + `Task.cancel()` 조합으로 0.3초 debounce 구현
...
Swift Testing Framework 기반 41개 테스트 케이스, 전체 통과.
...
| `SearchViewModelTests` | 7 | 검색 성공/실패, 북마크 토글 |
