...
# 유닛 + 통합 테스트 (UnitTests plan, UI 빌드 스킵)
xcodebuild -project KakaoImageSearch.xcodeproj -scheme KakaoImageSearch \
...
Swift Testing Framework 기반 175개 테스트 케이스를 작성했고, CI 및 로컬 기준으로 모두 통과했습니다.

| 테스트 Suite | 케이스 수 | 주요 검증 항목 |
|---|---|---|
| `ImageItemTests` | 22 | aspectRatio 경계값, Codable 라운드트립, Hashable, displayURL fallback, altText 생성 |
...
| `BookmarkCoordinatorTests` | 13 | load 성공/실패(throws), toggle 추가/제거, 연속 토글 상태 일관성, isBookmarked 판별, 동시 toggle dedup, 실패 시 optimistic 롤백, inFlight 정리, 동시 load 완료 대기, load 실패 전파, loaded 상태 재fetch 방지, refresh 강제 새로고침 |
...
Swift Testing Framework 기반 46개 테스트 케이스, CI 및 로컬 기준으로 모두 통과했습니다. (ImageAnalyzerIntegrationTests는 실기기 전용)
...
| `ImageAnalyzerIntegrationTests` | 2 | Vision API 키워드 추출 성공, 빈 이미지 에러 처리 (실기기 전용, 시뮬레이터 스킵) |
...
| Plan | 포함 타깃 | 용도 |
|------|----------|------|
| `UnitTests` | KakaoImageSearchTests (유닛+통합) | 로컬 개발, CI 기본 |
| `AllTests` | KakaoImageSearchTests + KakaoImageSearchUITests | 전체 검증 |
...
└── Presentation/
    ├── Coordinator/            # BookmarkCoordinator (Presentation 레이어 공유 상태)
