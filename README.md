...
| **ImageDownloader** | 메모리(NSCache) + 디스크 2단계 캐시, in-flight 중복 요청 dedup(호출자 취소 내성), Content-Type·크기 검증, prefetch 병렬도 제한(최대 6), URLSession 주입 가능 |
...
| `SearchViewModelTests` | 28 | 검색 성공/실패, 취소/race condition 처리, 페이지네이션, isEnd/page15 경계, 재시도, 북마크 토글/Toast, 연속 토글 dedup, loadMore/prefetch Task 취소 |
...
Swift Testing Framework 기반 42개 테스트 케이스, 작성한 테스트는 로컬 기준으로 모두 통과했습니다.
...
| `ImageDownloaderIntegrationTests` | 15 | MockImageURLProtocol 기반 다운로드 성공/실패, 캐시 히트, in-flight dedup(호출자 취소 내성), prefetch 병렬도 제한·부분 실패, Content-Type 검증, 크기 제한, http→https 변환 |
