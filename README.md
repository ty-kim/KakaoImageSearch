...
- `LazyVStack` 마지막 아이템 `.onAppear` 시점에 `loadMore()` 호출
...
Swift Testing Framework 기반 47개 테스트 케이스, 전체 통과.
...
| `SearchViewModelTests` | 14 | 검색 성공/실패, 페이지네이션, 재시도, 북마크 토글 |
| `BookmarkViewModelTests` | 3 | 목록 로드, 삭제 후 갱신 |
...
XCUITest 기반 21개 테스트 케이스, 전체 통과.

| 테스트 Suite | 케이스 수 | 주요 검증 항목 |
|---|---|---|
| `KakaoImageSearchUITests` | 11 | 앱 실행, 검색창 인터랙션, 탭 전환, 검색 결과 |
| `KakaoImageSearchRetryUITests` | 4 | 네트워크 오류 시 재시도 버튼 UX (`--simulateNetworkError`) |
