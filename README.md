...
Swift Testing Framework 기반 173개 테스트 케이스를 작성했고, CI 및 로컬 기준으로 모두 통과했습니다.
...
| `BookmarkStoreTests` | 13 | load 성공/실패(throws), toggle 추가/제거, 연속 토글 상태 일관성, isBookmarked 판별, 동시 toggle dedup, 실패 시 optimistic 롤백, inFlight 정리, 동시 load 완료 대기, load 실패 전파, loaded 상태 재fetch 방지, refresh 강제 새로고침 |
